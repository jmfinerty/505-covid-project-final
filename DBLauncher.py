...
def load_db():
    ...
    # Create Classes for three types of data and their derived attributes
    # based on requirements of the project

    # Data 1: Patient status messages from provided message exchange broker
    # (RabbitMQ)
    ...
    # Required by OF2:API search by mrn patients location (home or specific
    # hospital)
    ...
    # Required by RTR 1: API alert on zipcode that is in alert state based on
    # growth of postive cases.
    client.command("CREATE PROPERTY zipcodes.ziplist String")
    # Required by RTR 2: API alert on statewide when at least five zipcodes
    # are in alert state (based on RT1) within the same 15 second window.
    client.command("CREATE PROPERTY zipcodes.state_status Integer")
    # Required by RTR 3: API statewide positive and negative test counter
    client.command("CREATE PROPERTY zipcodes.positive_test Integer")
    # Required by RTR 3: API statewide positive and negative test counter
    client.command("CREATE PROPERTY zipcodes.negative_test Integer")
    ...
    # Required by OF3: API to report hospital patient numbers
    client.command("CREATE PROPERTY hospital.zipcode Integer")
    # Required by OF1: API to route pertson to the best fit hospital based on
    # patient status, provider status, distance (zipcode)
    client.command("CREATE PROPERTY hospital.level String")
    # Required by OF3: API to report hospital patient numbers
    client.command("CREATE PROPERTY hospital.total_beds Integer")
    client.command("CREATE PROPERTY hospital.state String")
    # Required by OF3: API to report hospital patient numbers
    client.command("CREATE PROPERTY hospital.avalable_beds Integer")
