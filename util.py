...
def read_patients_data(client, data):
    for item in data:
        ...
        if int(patient_status_code) in report_hospital or int(patient_status_code) in report_hospital_urgent:
            locs = client.query("SELECT zipcode FROM zipcodes WHERE zipcode = \"" + zipcode +"\"")
            if len(locs) != 0:
                client.command("UPDATE zipcodes INCREMENT positive_test = 1 WHERE zipcode = \"" + zipcode + "\"")
            else:
                client.command("CREATE VERTEX zipcodes SET zipcode = \"" + zipcode + "\", positive_test = 1, last_test = 0,"" ziplist = ' ', statealert = 0")
...
def read_hospital_data():
    ...
    # Now populate the hospital database with initialization total_beds == avalable_beds
    ...
    for i in range(len(hospital_data)):
        ...
        client.command("CREATE VERTEX hospital SET id = \"" + id + "\", name = \"" + str(name) + "\", state = \"" + state + "\", zipcode = \"" + zipcode + "\", level = \"" + level + "\", total_beds = \"" + total_beds + "\", avalable_beds = \"" + avalable_beds+"\"")
...
def findHospital(client,mrn,zipcode,patient_status_code):
    ...
    #patient is symptomatic and needs to be assigned to hospital
        if int(patient_status_code) in report_hospital_urgent:
            feasible_hospitals = client.command("SELECT * FROM hospital WHERE avalable_beds > 0"+ "\" AND level = 'LEVEL IV'")
            if len(feasible_hospitals) == 0: #IF NO LEVEL 4 Hospital found select at least normal hospital with available beds
                feasible_hospitals = client.command("SELECT * FROM hospital WHERE avalable_beds > 0")
        else:
            feasible_hospitals = client.command("SELECT * FROM hospital WHERE avalable_beds > 0")

        if len(feasible_hospitals) > 0 : #At least one hospital found
            best_hospital = findBestHospital(zipcode,feasible_hospitals)
            #Now assign this hospital and decrement the available bed counts of this hospital
            location_code = str(best_hospital.oRecordData['id'])
            client.command("UPDATE hospital INCREMENT avalable_beds = -1 WHERE id = \"" + location_code + "\"")
...
def findBestHospital(zipcode, feasible_hospitals):
    ...
    for hospital in feasible_hospitals:
        ...
        temp = float(distance_df[(distance_df['zip_from']==int(zipcode)) & (distance_df['zip_to']==int(hospital_zip))]['distance'].item())
        dist.append(temp)
